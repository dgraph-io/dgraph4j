  ...
  public void deleteEdgesTest() {
    ...
    Mutation mu =
        Mutation.newBuilder()
            ...
            .setCommitNow(true)
            .build();
    Response response = dgraphClient.newTransaction().mutate(mu);
    String uid = response.getUidsOrThrow("alice");
    ...
    Root root = gson.fromJson(response.getJson().toStringUtf8(), Root.class);
    assertEquals(root.me.get(0).friends.size(), 2);
    ...
    response = dgraphClient.newTransaction().query(query);
    root = gson.fromJson(response.getJson().toStringUtf8(), Root.class);
    assertNull(root.me.get(0).friends);
  }
