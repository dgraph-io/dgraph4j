...
public class GrpcDgraphClient implements DgraphClient {
    ...
    private GrpcDgraphClient(final String theHostname, final int thePort) {
        channel = ManagedChannelBuilder.forAddress(theHostname, thePort)
                                       .usePlaintext(true)
                                       .build();
    ...
    public GrpcDgraphResult query(final String theQuery) {
        try {
            final Graphresponse.Request aRequest = Graphresponse.Request.newBuilder()
                                                                        .setQuery(theQuery).build();
            logger.debug("Sending request to Dgraph...");
            final Graphresponse.Response aResponse = blockingStub.query(aRequest);
