...
**Warning**: The Maven build runs integration tests on a locally running Dgraph server.
...
### Code Style
We use [google-java-format] to format the source code. If you run `mvn clean install`, you will be
warned if there is code that is not conformant. You can run `mvn google-java-format:format` to format the source
